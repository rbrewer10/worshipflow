...
**Correction (post-Task-7, found by code review):** Task 7's own header claimed `ZonePinPicker.tsx` was "Live tab, in scope." That was wrong — tracing actual imports shows `ZonePinPicker.tsx` is mounted only by `ZoneLiveGrid.tsx`, which is mounted only by `ZonePanel.tsx` (Setup's "Screens & zones" tab), never by anything the Live tab renders. `LiveZoneStatus.tsx`'s own comment ("No pin controls here — pinning stays a Setup-only action") already said as much; this plan should have caught it during scope-drafting and didn't. Task 7 as implemented is not a bug — `ZonePinPicker.tsx`'s whole popover (not just `ZoneTrackToggle.tsx`'s small toggle, as originally anticipated below) now renders fully dark, correctly and consistently within itself, but it only ever opens from Setup's still-fully-light zone grid, ahead of Setup's own migration stage. This is a larger instance of the same "shared infrastructure ripples early" tradeoff the Foundation stage's `main.css` classes already established — kept as-is (reverting reviewed, tested, working code over a scope-label mistake isn't worth the churn), documented here so a future contributor doesn't look for this popover on the Live tab or assume it got Live-tab exposure/testing it never had.

**Original (inaccurate) framing, left for context:** "One shared component gets touched early, deliberately: `ZoneTrackToggle.tsx` (Task 7) is shared between `ZonePinPicker.tsx` (Live tab, in scope) and `ZonePanel.tsx` (Setup, not yet migrated). Migrating it now will make Setup's zone-track toggle render in dark colors too, ahead of Setup's own stage — this is the same 'shared infrastructure updates once, ripples everywhere' pattern the Foundation stage's `main.css` classes already established... Accept this; it's expected, not a bug to fix later." — correct in spirit, wrong about `ZonePinPicker.tsx`'s own scope.
