...
function PptxImport({ onImported }: { onImported: () => void }): JSX.Element {
  ...
  return (
    <>
      <button
        onClick={pick}
        className="mb-3 w-full rounded-lg border border-blue-500/30 bg-blue-600/20 px-3 py-2 text-sm font-semibold text-blue-200 hover:bg-blue-600/30"
      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-6" onClick={close}>
          ...
          >
            <div className="mb-1 flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-200">Import {parsed.length} PowerPoint file{parsed.length === 1 ? '' : 's'}</h3>
              <button onClick={close} className="rounded bg-white/10 px-3 py-1 text-xs font-semibold text-slate-300 hover:bg-white/20">Cancel</button>
            </div>
                ...
                <div key={i} className="rounded-lg border border-white/[0.07] bg-black/30 p-2">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="min-w-0 flex-1 rounded border border-white/10 bg-black/40 px-2 py-1 text-sm font-medium text-slate-200 outline-none focus:border-blue-500"
                    />
                  ...
                  {song.slides.length === 0 && (
                    <div className="mt-1 pl-6 text-xs text-amber-400">No text found in this file — it may be image-based slides.</div>
                  )}
                </div>
              ))}
            </div>
