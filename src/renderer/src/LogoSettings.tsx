...
function LogoSettings(): JSX.Element {
  ...
  return (
    <div className="h-full overflow-auto p-6">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-content-primary">Logo &amp; Background</h1>
      ...
      <div className="mx-auto max-w-xl space-y-5">
        ...
        <div className="rounded-xl border border-border bg-panel p-5">
          <h2 className="font-semibold text-content-primary">Church Name</h2>
          <div className="text-xs text-content-secondary mt-0.5 mb-3">Shown on logo / idle screens when no logo image is set.</div>
          <input
            type="text"
            value={churchName}
            onChange={(e) => saveChurchName(e.target.value)}
            placeholder="e.g. Snow Hill Church"
            className="w-full rounded-lg border border-border px-3 py-2 text-sm text-content-primary focus:border-blue-400 focus:outline-none"
          />
        </div>
        ...
        <div className="rounded-xl border border-border bg-panel p-5">
          ...
          {/* Logo preview */}
          <div className="mb-4 flex aspect-video w-full items-center justify-center overflow-hidden rounded-lg border border-border bg-slate-900">
            {logoPath ? (
              <img
          ...
          {logoPath && (
            <div className="mb-3 flex items-center gap-2 rounded-lg bg-panel-raised border border-border px-3 py-2">
              <ImageIcon size={14} className="shrink-0 text-content-secondary" />
              <span className="min-w-0 flex-1 truncate text-xs text-content-secondary">{logoName}</span>
            </div>
          )}
          ...
          <div className="flex gap-2">
            <button
              onClick={pickLogo}
              className="flex-1 rounded-lg border border-blue-500/25 bg-blue-500/10 px-4 py-2.5 text-sm font-semibold text-blue-400 hover:bg-blue-500/20"
            >
            ...
            {logoPath && (
              <button
                onClick={removeLogo}
                className="rounded-lg border border-border bg-panel-raised px-3 py-2 text-sm font-semibold text-content-secondary hover:bg-border-strong"
              >
                Remove
              </button>
            )}
        ...
        <div className="rounded-xl border border-border bg-panel p-5">
          <div className="mb-4">
            <h2 className="font-semibold text-content-primary">Motion Background</h2>
          ...
          {logoBg && (
            <div className="mb-3 flex items-center gap-2 rounded-lg bg-panel-raised border border-border px-3 py-2">
              {isVideoBg ? <Film size={14} className="shrink-0 text-content-secondary" /> : <ImageIcon size={14} className="shrink-0 text-content-secondary" />}
              <span className="min-w-0 flex-1 truncate text-xs text-content-secondary">{bgName}</span>
            </div>
          )}
        ...
        <div className="rounded-xl border border-border bg-panel p-5">
          <div className="mb-4">
            <h2 className="font-semibold text-content-primary">Screen Scale</h2>
            <div className="text-xs text-content-secondary mt-0.5">
          ...
          <div className="space-y-3">
            {(Object.keys(ZONE_LABELS) as unknown as ZoneId[]).map((zoneId) => (
              <div key={zoneId} className="flex items-center gap-3">
                <span className="w-32 shrink-0 text-xs font-medium text-content-secondary">{ZONE_LABELS[zoneId]}</span>
                <input
                  type="range"
                  min={50}
                  max={150}
                ...
                />
                <span className="w-12 shrink-0 text-right text-xs font-mono tabular-nums text-content-secondary">
