            ...
            {FONT_SIZES.map((s) => (
              <button
                ...
                className={`rounded-full px-2 py-0.5 text-[11px] font-semibold transition-colors ${
                  (song.fontScale ?? 4) === s ? 'bg-indigo-500 text-white' : 'text-white/60 hover:bg-white/10 hover:text-white'
                }`}
              >
                {s}
              </button>
        ...
        {editing && (
          <div className="absolute inset-0 flex items-center justify-center px-[8%] py-[6%]">
            <textarea
              ...
              className="w-full resize-none overflow-hidden rounded bg-transparent text-center font-bold leading-tight outline-none ring-2 ring-indigo-400/60"
              style={{
                fontSize: fontSizePx,
                fontFamily,
                color: textColor,
